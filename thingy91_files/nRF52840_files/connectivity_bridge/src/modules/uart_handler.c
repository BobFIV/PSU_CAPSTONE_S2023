...
#define BACKEND_PARSE_BUFFER_SIZE 7000
...
void parse_nrF9160_char(char c) {
	...
	if (backend_started && c != ';') {
		...
		/*if (strncmp(backend_parse_buf, "C", BACKEND_PARSE_BUFFER_SIZE) == 0) {
			...
			//b->backend_parse_buf = backend_parse_buf;
			memcpy(b->backend_parse_buf, backend_parse_buf, sizeof(backend_parse_buf));
			APP_EVENT_SUBMIT(b);
		}*/
...
static int uart_tx_start(uint8_t dev_idx)
{
	...
	*data = 123456;
	...
	//uart_tx_ringbufs[dev_idx].rb = "Test\n\r";
